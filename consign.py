...
class ConsignOrder(models.Model):
    _name = "consign.order"
    ...
    def get_empty_list_help(self, help):
        ...
        return super(ConsignOrder, self).get_empty_list_help(help)
    ...
    order_line = fields.One2many('.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True, auto_join=True)
    ...
    company_id = fields.Many2one('res.company', 'Company', default=lambda self: self.env['res.company']._company_default_get('consign.order'))
    ...
    def _compute_portal_url(self):
        super(ConsignOrder, self)._compute_portal_url()
        for order in self:
    ...
    def unlink(self):
        ...
        return super(ConsignOrder, self).unlink()
    ...
    def _track_subtype(self, init_values):
        ...
        return super(ConsignOrder, self)._track_subtype(init_values)
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            if 'company_id' in vals:
                vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code('consign.order') or _('New')
            else:
                vals['name'] = self.env['ir.sequence'].next_by_code('consign.order') or _('New')
        ...
        result = super(ConsignOrder, self).create(vals)
        return result
    ...
    def copy_data(self, default=None):
        ...
        return super(ConsignOrder, self).copy_data(default)
    ...
    def name_get(self):
        if self._context.get('sale_show_partner_name'):
            ...
            return res
        return super(ConsignOrder, self).name_get()
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        ...
        return super(ConsignOrder, self).name_search(name, args, operator, limit)
    ...
    def _init_column(self, column_name):
        ...
        if column_name != 'access_token':
            super(ConsignOrder, self)._init_column(column_name)
        else:
    ...
    def get_access_action(self, access_uid=None):
        ...
        return super(ConsignOrder, self).get_access_action(access_uid)
    ...
    def _notification_recipients(self, message, groups):
        groups = super(ConsignOrder, self)._notification_recipients(message, groups)
...
class ConsignOrderLine(models.Model):
    _name = 'consign.order.line'
    ...
    def create(self, values):
        ...
        line = super(ConsignOrderLine, self).create(values)
    ...
    def write(self, values):
        ...
        result = super(ConsignOrderLine, self).write(values)
        return result

    order_id = fields.Many2one('consign.order', string='Order Reference', required=True, ondelete='cascade', index=True, copy=False)
    ...
    def _get_display_price(self, product):
        # TO DO: move me in master/saas-16 on consign.order
    ...
    def name_get(self):
        if self._context.get('sale_show_order_product_name'):
            ...
            return result
        return super(ConsignOrderLine, self).name_get()
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        if self._context.get('sale_show_order_product_name'):
            if operator in ('ilike', 'like', '=', '=like', '=ilike'):
                ...
                return self.search(domain, limit=limit).name_get()
        return super(ConsignOrderLine, self).name_search(name, args, operator, limit)

    @api.multi
    def unlink(self):
        if self.filtered(lambda x: x.state in ('sale', 'done')):
            raise UserError(_('You can not remove a sales order line.\nDiscard changes and try setting the quantity to 0.'))
        return super(ConsignOrderLine, self).unlink()
